        ...
        <div className="grid items-center gap-14 py-12 lg:grid-cols-[minmax(0,1fr)_460px] lg:gap-24 lg:py-16">
          ...
          <section className="rounded-[1.75rem] border border-white/10 bg-[#0b1726]/90 p-6 shadow-2xl shadow-black/30 backdrop-blur-xl sm:p-8">
            ...
            {isRegistration && registrationStep === 2 ? (
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {planOptions.map((plan) => <button key={plan.name} type="button" onClick={() => setSelectedPlan(plan.name)} className={`rounded-xl border p-3 text-left transition-colors ${selectedPlan === plan.name ? 'border-[#00E676] bg-[#00E676]/15 text-[#00E676]' : 'border-white/10 bg-white/[0.03] hover:border-white/30'}`}><span className="block text-sm font-bold">{plan.name}</span><span className="mt-1 block text-xs text-white/50">{plan.price}</span></button>)}
                  </div>
                </div>
                <div>
                  <p className="mb-3 text-sm font-medium text-white/75">Elige tus clases</p>
                  <div className="grid grid-cols-2 gap-2">
                    {classOptions.map((className) => { const isSelected = selectedClasses.includes(className); return <button key={className} type="button" onClick={() => setSelectedClasses((current) => isSelected ? current.filter((item) => item !== className) : [...current, className])} className={`rounded-xl border px-3 py-3 text-left text-sm transition-colors ${isSelected ? 'border-[#00E676] bg-[#00E676]/15 text-[#00E676] font-bold' : 'border-white/10 bg-white/[0.03] text-white/70 hover:border-white/30'}`}>{isSelected ? '✓ ' : ''}{className}</button>; })}
                  </div>
                </div>
                {formError && <p role="alert" className="text-sm text-rose-300">{formError}</p>}
                <button type="button" disabled={isSubmitted} onClick={completeRegistration} className="group flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#00E676] font-bold text-[#030f1d] disabled:opacity-70 shadow-lg shadow-[#00E676]/20 hover:bg-[#00E676]/90 transition-all">{isSubmitted ? 'Creando tu perfil...' : 'Entrar a mi cuenta'}{!isSubmitted && <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />}</button>
              </div>
            ) : <form onSubmit={handleSubmit} className="space-y-4">
              {isRegistration && (
                <label className="block space-y-2 text-sm font-medium text-white/75">Nombre completo
                  <span className="relative block"><UserRound className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/35" /><input name="name" type="text" placeholder="Tu nombre" className="h-12 w-full rounded-xl border border-white/10 bg-white/[0.04] pl-10 pr-4 text-white outline-none transition-colors placeholder:text-white/25 focus:border-[#00E676]/70" /></span>
                </label>
              )}
              ...
              {!isRegistration && <div className="flex justify-end"><button type="button" className="text-xs font-semibold text-[#00E676] hover:underline">Olvidé mi contraseña</button></div>}
              {formError && (
