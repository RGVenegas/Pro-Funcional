      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-white/5 border border-white/10 rounded-lg text-white placeholder:text-white/40 focus:outline-none focus:border-[#00E676]"
          />
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
              filterStatus === 'all' ? 'bg-[#00E676] text-[#021826] font-bold' : 'bg-white/5 text-white hover:bg-white/10'
            }`}
          >
            Todos
          </button>
          <button
            ...
            className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
              filterStatus === 'active' ? 'bg-[#00E676] text-[#021826] font-bold' : 'bg-white/5 text-white hover:bg-white/10'
            }`}
          >
            Activos
          </button>
          <button
            ...
            className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${
              filterStatus === 'expired' ? 'bg-[#00E676] text-[#021826] font-bold' : 'bg-white/5 text-white hover:bg-white/10'
            }`}
          >
            Vencidos
          </button>
              ...
              {filteredMembers.map((member) => (
                <tr key={member.id} className="cursor-pointer border-b border-white/5 hover:bg-white/5 transition-colors" onClick={() => onViewMember(member.id)}>
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <button type="button" className="font-medium truncate text-left text-[#F7F7F7] hover:text-[#00E676]" onClick={() => onViewMember(member.id)}>{member.name}</button>
                  ...
                  <td className="p-4 hidden lg:table-cell">
                    <span className={member.balance < 0 ? 'text-red-400' : member.balance > 0 ? 'text-[#00E676]' : 'text-white/60'}>
                  ...
                  <td className="p-4 text-right">
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-3 py-2 bg-[#00E676] text-[#021826] font-bold rounded-lg hover:bg-[#00E676]/90 transition-colors"
                    >
